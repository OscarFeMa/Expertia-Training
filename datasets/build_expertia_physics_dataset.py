...
DEFAULT_OUT = r"D:\proyectos\expertia\training\datasets\expertia-physics-puro.jsonl"
SYSTEM_PROMPT = "Eres ExpertiaPhysics, fisico puro. Responde solo con definicion formal y formula cuando aplique. Sin opinion web."
BATCH = 2000
...
def to_record(topic, output, qid, source_url, origin):
    ...
    return {
        ...
        "input": "",
        "output": output[:2000],
        "metadata": {"domain": "Physics", "qid": qid, "source_url": source_url, "origin": origin},
    }


def load_raw(limit_each=15000):
    recs, seen = [], set()
    f = RAW / "sparql_p2534.jsonl"
    if f.exists():
        for line in open(f, encoding="utf-8"):
            try:
                ...
                continue
            if not phys_ok(r.get("instance_of")):
                continue
            key = ("q", r.get("qid"))
            if key in seen:
                continue
            ...
            if len(recs) >= limit_each:
                break
    for name, origin in (("se_physics.jsonl", "se_physics"), ("wiki_physics.jsonl", "wiki_physics")):
        f = RAW / name
        if not f.exists():
            continue
        for line in open(f, encoding="utf-8"):
    ...
    try:
        if max_id is None:
            rows = con.execute(
                ...
                "WHERE domain='Physics' AND qid IS NOT NULL "
                "ORDER BY id DESC LIMIT ?", (batch,)).fetchall()
        else:
            rows = con.execute(
                "SELECT id, qid, topic, structured_knowledge, source_url FROM knowledge_packages "
                "WHERE domain='Physics' AND qid IS NOT NULL AND id < ? "
