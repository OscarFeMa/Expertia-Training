...
DEFAULT_OUT = r"D:\proyectos\expertia\training\datasets\expertia-swe-puro.jsonl"
AUDIT = RAW / "audit_swe.jsonl"
SYSTEM_PROMPT = "Eres ExpertiaSoftwareEngineering, ingeniero de software puro. Responde solo con definicion formal, parametros y ejemplo minimo cuando aplique. Sin opinion web."
BATCH = 2000
AUDIT_MIN = 6
...
def to_record(topic, output, qid, source_url, origin):
    ...
    return {
        ...
        "metadata": {"domain": "SoftwareEngineering", "qid": qid, "source_url": source_url, "origin": origin},
    }
...
def load_raw():
    ...
    for name, origin in (("se_stackoverflow.jsonl", "se_stackoverflow"),
                         ("se_softwareengineering.jsonl", "se_softwareengineering"),
                         ("se_codereview.jsonl", "se_codereview")):
        f = RAW / name
        if not f.exists():
    ...
    if f.exists():
        ...
        for line in open(f, encoding="utf-8"):
            ...
            if audit is not None and audit.get(str(r.get("qid")), -1) < AUDIT_MIN:
                n_drop += 1
                continue
            rec = to_record(r.get("label"), r.get("formula"), r.get("qid"), "", "wiki_swe")
            if rec:
                mark(r.get("qid"), r.get("label"))
                recs.append(rec)
                n_ok += 1
        print("wiki_swe: +%d (filtrados auditoria: %d)" % (n_ok, n_drop))
    else:
        print("WARN falta wiki_swe.jsonl")
    return recs, seen
    ...
    try:
        if max_id is None:
            rows = con.execute(
                ...
                "WHERE domain='SoftwareEngineering' AND qid IS NOT NULL "
                "ORDER BY id DESC LIMIT ?", (batch,)).fetchall()
        else:
            rows = con.execute(
                "SELECT id, qid, topic, structured_knowledge, source_url FROM knowledge_packages "
                "WHERE domain='SoftwareEngineering' AND qid IS NOT NULL AND id < ? "
