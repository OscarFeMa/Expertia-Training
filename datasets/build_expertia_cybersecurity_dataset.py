...
DEFAULT_OUT = r"D:\proyectos\expertia\training\datasets\expertia-cybersecurity-puro.jsonl"
AUDIT = RAW / "audit_cybersecurity.jsonl"
SYSTEM_PROMPT = "Eres ExpertiaCybersecurity, experto en ciberseguridad puro. Responde solo con definicion formal, parametros y ejemplo minimo cuando aplique. Sin opinion web."
BATCH = 2000
AUDIT_MIN = 6
...
def to_record(topic, output, qid, source_url, origin):
    ...
    return {
        ...
        "input": "",
        "output": output[:2000],
        "metadata": {"domain": "Cybersecurity", "qid": qid, "source_url": source_url, "origin": origin},
    }
...
def load_raw():
    ...
    for name, origin in (("se_security.jsonl", "se_security"),
                         ("se_crypto.jsonl", "se_crypto"),
                         ("se_networkengineering.jsonl", "se_networkengineering")):
        f = RAW / name
        if not f.exists():
        ...
        for line in open(f, encoding="utf-8"):
            ...
            if rec:
                ...
                n += 1
        print("%s: +%d" % (name, n))
    f = RAW / "wiki_cybersecurity.jsonl"
    if f.exists():
        n_ok = n_drop = 0
        for line in open(f, encoding="utf-8"):
            try:
            ...
            if audit is not None and audit.get(str(r.get("qid")), -1) < AUDIT_MIN:
                n_drop += 1
                continue
            rec = to_record(r.get("label"), r.get("formula"), r.get("qid"), "", "wiki_cybersecurity")
            if rec:
                mark(r.get("qid"), r.get("label"))
                recs.append(rec)
                n_ok += 1
        print("wiki_cybersecurity: +%d (filtrados auditoria: %d)" % (n_ok, n_drop))
    else:
        print("WARN falta wiki_cybersecurity.jsonl")
    return recs, seen
    ...
    try:
        if max_id is None:
            rows = con.execute(
                ...
                "WHERE domain='Cybersecurity' AND qid IS NOT NULL "
                "ORDER BY id DESC LIMIT ?", (batch,)).fetchall()
        else:
            rows = con.execute(
                "SELECT id, qid, topic, structured_knowledge, source_url FROM knowledge_packages "
                "WHERE domain='Cybersecurity' AND qid IS NOT NULL AND id < ? "
