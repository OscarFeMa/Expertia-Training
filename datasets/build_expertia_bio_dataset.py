...
DEFAULT_OUT = r"D:\proyectos\expertia\training\datasets\expertia-bio-puro.jsonl"
AUDIT = RAW / "audit_bio.jsonl"
SYSTEM_PROMPT = "Eres ExpertiaBiology, biologo puro. Responde solo con definicion formal, parametros y ejemplo cuando aplique. Sin opinion web."
BATCH = 2000
AUDIT_MIN = 6
    ...
    if f.exists():
        ...
        for line in open(f, encoding="utf-8"):
            ...
            if audit is not None and audit.get(str(r.get("qid")), -1) < AUDIT_MIN:
                n_drop += 1
                continue
            rec = to_record(r.get("label"), r.get("formula"), r.get("qid"), "", "wiki_bio")
            if rec:
                mark(r.get("qid"), r.get("label"))
                recs.append(rec)
                n_ok += 1
        print("wiki_bio: +%d (filtrados auditoria: %d)" % (n_ok, n_drop))
    else:
        print("WARN falta wiki_bio.jsonl")
    return recs, seen
    ...
    try:
        if max_id is None:
            rows = con.execute(
                ...
                "WHERE domain='Biology' AND qid IS NOT NULL "
                "ORDER BY id DESC LIMIT ?", (batch,)).fetchall()
        else:
            rows = con.execute(
                "SELECT id, qid, topic, structured_knowledge, source_url FROM knowledge_packages "
                "WHERE domain='Biology' AND qid IS NOT NULL AND id < ? "
